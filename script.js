...
class GameOfWar {
  ...
  war(){
    if (this.p1.length < 4 || this.p2.length < 4){
      if (this.p1.length < 4) {
        // p1 loses
        this.p2.push(...this.p1.splice(0, this.p1.length))
      } else {
        // p2 loses
        this.p1.push(...this.p2.splice(0, this.p2.length))
      }
    } else {
  ...
  play(){
    ...
    if (this.p1.length > 0) {
      console.log(`Player 1 has won!!! They have ${this.p1.length} cards in their deck`)
    } else {
      console.log(`Player 2 has won!!! They have ${this.p2.length} cards in their deck`)
    }
  }
}
